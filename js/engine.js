...
export function tick(dieFn) {
  ...
  for (let i = G.enemies.length - 1; i >= 0; i--) {
    ...
    if (en.body[0].x === head.x && en.body[0].y === head.y) {
      if (isInvincible || isShielded || (G.rogueFang||0) > 0) {
        killEnemy(i);
        ...
        if (en.type === 'bomber') {
          bomberExplosion(en.body[0].x, en.body[0].y);
        }
        return handleHit('撞上敌人蛇头', dieFn);
      }
    }
    // ★ 撞敌蛇身体 → 直接击杀
  ...
  // ★ Boss 战无碰撞卡死深度优化（无敌状态时直接无阻力穿透并进行 dmg 扣减）
  if (G.boss && !(G.bossType === 'ghost' && G.boss.phased)) {
    ...
    if (head.x === bh.x && head.y === bh.y) {
      if (isInvincible || isShielded) {
        ...
        // ★ 优化：直接让 tick 继续，平滑走位穿透过去，不在这里 unshift or return !
      } else {
